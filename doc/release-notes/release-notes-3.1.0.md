sQuorum Core version *3.1.0* is now available from:  <https://github.com/akcryptoguy/squorum/releases>
...
zSQR Staking is here! zSQR staking will be activated on the morning of the 8th of May 2018. With the release of zSQR staking, there are effectively 2 versions of zSQR, zSQR minted on the 3.0.6 Helium wallet or lower, and zSQR minted on Helium wallet version or higher. New features in this release will require the use of zSQR v2, zSQR minted on this wallet release 3.1.0 or later. If you currently hold zSQR v1 and wish to take advantage of zSQR staking and deterministic zSQR, you will need to spend the zSQR v1 to yourself and remint zSQR v2.
...
To encourage the use of zSQR and increase the Helium zerocoin anonymity set, the Helium payment schedule has been changed to the following:
...
The visual layout of the Helium Qt wallet has undergone a near-complete overhaul.
...
### Summary
 ...
 The Helium wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the squorum.conf file, found in the Helium data directory.
...
Helium Daemon & Client (RPC Changes)
--------------

### RPC Ban Management

The Helium client peer bans now have additional RPC commands to manage peers. Peers can be banned and unbanned at will without the need to restart the wallet client. No changes have been made to the conditions resulting in automatic peer bans. New RPC commands: `setban`, `listbanned`, `clearbanned`, and `disconnectnode`
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the Helium support group, which often times sees users say "I didn't spend that zSQR". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the Helium masternodes, the monthly budget available to be utilised has been increased to 42,000 SQR / month. This SQR only has the opportunity to be raised once per month (paid to winning proposals) with any unused SQR not created by the blockchain.

### Budget Finalization Fee

The Helium finalization fee for successful proposals has now been reduced, this fee is now 5 SQR down from 50 SQR. The total fee outlay for a successful proposal is now a total of 55 SQR.
...
Several critical security flaws in the zerocoin protocol and Helium's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zSQR Control dialog in the QT wallet, a user is able to see zSQR marked as version 1 or 2.
...
Helium's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which Helium used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to Helium using the bug bounty program. The modulus is now correctly imported and Helium's accumulators have been changed to use the new proper modulus.
